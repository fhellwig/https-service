      ...
      response.on('end', _ => {
        ...
        if (chunks.length > 0) {
          ...
          if (type === JSON_MEDIA_TYPE) {
            ...
            if (!json) {
              return reject(new ServerError(500, 'Server returned an empty response.'));
            }
            try {
              data = JSON.parse(json);
            } catch (e) {
              return reject(new ServerError(500, `Cannot parse response (${e.message}).`));
            }
        ...
        reject(new ServerError(code, errorMessage(type, data)));
      });
    });
